...
function getOptions() {
  $.ajax({
    url: "https://toddles-api.phantominteractive.com.au/options",
    type: "GET",
    headers: {
...
function process() {
  ...
  if (selectedOptions && inputQuery) {
    ...
    $.ajax({
      url: "https://toddles-api.phantominteractive.com.au/process",
      type: "POST",
      data: JSON.stringify({
...
function deleteClient() {
  ...
  $.ajax({
    url: "https://toddles-api.phantominteractive.com.au/client_delete",
    type: "POST",
    data: JSON.stringify({
      client_id: clientId,
    }),
    ...
    success: function (response) {
      options = options.filter((client) => client.id !== clientId);
      genHtml(options);
    },
    error: function (jqXHR, exception) {
      redirectAjaxError(jqXHR);
...
function getPrompt(blockName) {
  return new Promise(function (resolve, reject) {
    if (blockName) {
      $.ajax({
        url: `https://toddles-api.phantominteractive.com.au/prompt?query=${blockName}`,
        timeout: 60000,
        type: "GET",
        headers: {
          Authorization: getAuthHeader(),
